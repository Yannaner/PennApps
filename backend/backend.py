# Physical-witness coordinator for 2 UNO nodes
import time, random, serial, threading, sys

PORTS = ["/dev/tty.usbmodem2101", "/dev/tty.usbmodem48CA436008D02"]
BAUD  = 115200
DURATION_MS = 3000
THRESH = 0.005          # correlation threshold
ROUND_PAUSE = 0.5
TXS_PER_BLOCK = 3
...
def read_lines(i):
    """Non-blocking line reader for node i"""
    global buf
    s = ser[i]
    while True:
        b = s.read(1)
...
def handle_line(i, line):
    ...
    if line.startswith("WIT"):
        # Parse witness response: WIT round=12 node=1 corr=0.823
        parts = {}
...
def include_txs_and_apply():
    """Apply up to TXS_PER_BLOCK transactions from mempool to balances"""
    included = []
...
def tx_input_thread():
    print("Type transactions like: send Alice Bob 3")
    ...
    while True:
        try:
            ...
            if parts[0].lower()=="send" and len(parts)==4:
                ...
                print("Unknown command. Try: send Alice Bob 3 | mint Bob 5 | bal")
...
def main():
    ...
    print("Coordinator running. Alternating leaders. Ctrl+C to stop.")
    try:
        while True:
            for leader in (0, 1):
                cur_round += 1
                ...
                if witness_ok:
                    # Commit block: include transactions and update balances
                    ...
                    print(f"Block {cur_round} skipped (no witness).")
